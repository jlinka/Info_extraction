#!/usr/bin/env python
# -*- coding: utf-8 -*-
# @Time    : 2019/3/25 上午10:08
# @Author  : jlinka
# @File    : bert_extract.py
...
class Bert_ExtractPreprocess(object):
    _total_labels = ['0', '1', '2', '3']
    ...
    def load_traindata(self):
        try:
            wtrain_x = np.load(config.PREDATA_DIC + '/bert_etrain_x.npy')
            wtrain_y = np.load(config.PREDATA_DIC + '/bert_etrain_y.npy')
    ...
    def load_testdata(self):
        try:
            wtest_x = np.load(config.PREDATA_DIC + '/bert_etest_x.npy')
            wtest_y = np.load(config.PREDATA_DIC + '/bert_etest_y.npy')
    ...
    def _split_tagdata(self, datas: list):
        ...
        for news in datas:
            ...
            for sentence in news:
                if flag == 0:
                    words.append(sentence[6:].split(','))
                    flag = 1
                else:
                    flag = 0
                    labels.append(sentence[3:].split(','))
...
bert_wpreprocess = Bert_ExtractPreprocess()

if __name__ == '__main__':
    a, b, c, d = bert_wpreprocess.deal_tagdata(os.listdir(config.PRE_DIC))
    np.save(config.PREDATA_DIC + '/bert_etrain_x.npy', np.array(a))
    np.save(config.PREDATA_DIC + '/bert_etrain_y.npy', np.array(b))
    np.save(config.PREDATA_DIC + '/bert_etest_x.npy', np.array(c))
    np.save(config.PREDATA_DIC + '/bert_etest_y.npy', np.array(d))
