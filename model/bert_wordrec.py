#!/usr/bin/env python
# -*- coding: utf-8 -*-
# @Time    : 2019/3/5 上午10:11
# @Author  : jlinka
# @File    : bert_wordrec.py
...
# from preprocess.bert_wordpre_test import preprocess as ERROR:[Errno 2] No such file or directory
...
class BertWordRecModel(object):
    def __init__(self, sentence_len: int = config.SENTENCE_LEN, wordvec_size: int = config.BERT_EMBEDDING_SIZE,
                 classes: int = len(bert_wpreprocess.get_total_labels()),
    ...
    def train(self, epochs: int = config.WR_EPOCHS, batch_size: int = config.WR_BATCH_SIZE,
        ...
        with sess.graph.as_default():
            ...
            plt.ylabel("f1_conll")
            lval_f1, = plt.plot(x, y, label='val_f1')
            lval_recall, = plt.plot(x, q, label='val_recall')
            lval_precision, = plt.plot(x, z, label='val_precision')
            plt.legend(handles=[lval_f1, lval_recall, lval_precision])
    ...
    def _epoch_train(self, sess: tf.Session, ph_sequence_lengths, ph_x, ph_y, train_opt, pred, batch_size: int,
        ...
        if generator is None:
            for train_x, train_y in bert_wpreprocess.get_batch_traindata(batch_size):
    ...
    def test(self, batch_size: int = config.SR_BATCH_SIZE, generator=None):
        ...
        with sess.graph.as_default():
            ...
            test_true_y, test_pred_y = self._epoch_test(sess, ph_sequence_lengths, ph_x, ph_y, outputs, batch_size,
                                                        generator)
            print(test_true_y, test_pred_y, bert_wpreprocess.get_total_labels())
            default_evaluate.print_evaluate(test_true_y, test_pred_y, bert_wpreprocess.get_total_labels())
    ...
    def _epoch_test(self, sess: tf.Session, ph_sequence_lengths, ph_x, ph_y, pred, batch_size: int, generator=None):
        ...
        if generator is None:
            for test_x, test_y in bert_wpreprocess.get_batch_testdata(batch_size):
    ...
    def predict_label(self, inputs):
        ...
        for indexs in pred_y:
            labels = list()
            for index in indexs:
                labels.append(bert_wpreprocess.get_label(index))
            labels_list.append(labels)
        return labels_list


bert_default_model = BertWordRecModel()
